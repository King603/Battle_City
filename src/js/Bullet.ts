...
export class Bullet extends Obj {
	 ...
	 * @param {Number} dir 
	 */
	constructor(public ctx: CanvasRenderingContext2D, public owner: Tank, public type: number, public dir: number) {
		super(6, 3);
	}
	hit = !1;
	isDestroyed = !1;
	draw() {
		let sx = (POS.bullet[0] as number) + this.dir * this.size;
		let sy = POS.bullet[1] as number;
	...
	isHit() {
		...
		if (!this.hit) {
			...
			switch (this.type) {
				case PLAYER:
					if (enemyArray != null || (enemyArray as (Enemy3 | Enemy1 | Enemy2)[]).length > 0) {
