...
import { AUDIO, CRACK_TYPE, DIR } from "./const.js";
import { CrackAnimation } from "./CrackAnimation.js";
import { bulletArray, crackArray, map, time } from "./main.js";
...
export class Tank extends Obj {
	...
	bullet: Bullet = null as unknown as Bullet; // 子弹
	shootRate = .6; // 射击的概率
	tempX = 0;
	tempY = 0;
	move() {
		// 如果是AI坦克，在一定时间或者碰撞之后切换方法
		if (this.isAI && time.emenyStop > 0)
			return;
		this.tempX = this.x;
		this.tempY = this.y;
		if (this.isAI) {
			...
			if (this.frame % 100 == 0 || this.hit) {
				this.dir = parseInt(Math.random() * 4 as unknown as string); // 随机一个方向
				this.hit = !1;
				this.frame = 0;
			}
		}
	 ...
	 */
	shoot(type: number) {
		if (this.isAI && time.emenyStop > 0 || this.isShooting)
			return;
		let tempX = this.x;
		let tempY = this.y;
		this.bullet = new Bullet(this.ctx, this, type, this.dir);
		let { size } = this.bullet;
		let difference = parseInt(this.size / 2 as unknown as string) - parseInt(size / 2 as unknown as string);
		switch (this.dir) {
