...
export let ctx: MyCtxOBJ = {};
export let menu: Menu;// 菜单
export let stage: Stage;// 舞台
export let map: Map;// 地图
export let player: PlayTank[] = [];
export let prop: Prop;
export let enemyArray: (Enemy1 | Enemy2 | Enemy3)[] = [];// 敌方坦克
export let bulletArray: Bullet[] = [];// 子弹数组
let keys: number[] = [];// 记录按下的按键
export let crackArray: CrackAnimation[] = [];// 爆炸数组
...
function initObject() {
	menu = new Menu((ctx as MyCtxOBJ).stage);
	stage = new Stage((ctx as MyCtxOBJ).stage, level);
	map = new Map((ctx as MyCtxOBJ).wall, (ctx as MyCtxOBJ).grass);
	player[0] = new PlayTank((ctx as MyCtxOBJ).tank);
	player[0].x = 129 + map.offsetX;
	player[0].y = 385 + map.offsetY;
	player[1] = new PlayTank((ctx as MyCtxOBJ).tank);
	...
	overX = 176;
	overY = 384;
	(ctx as MyCtxOBJ).over.clearRect(0, 0, SCREEN.WIDTH, SCREEN.HEIGHT);
	time.emenyStop = 0;
...
body.keydown(({ keyCode }) => {
	switch (gameState) {
		...
		case GAME_STATE.START:
			(keys as MyArray).contain(keyCode) || keys.push(keyCode);
			// 射击
			switch (keyCode) {
...
body.keyup(({ keyCode }) => (keys as MyArray).remove(keyCode));
	...
	if (bulletArray != null && bulletArray.length > 0) {
		for (let i = 0; i < bulletArray.length; i++) {
			...
			if (bulletObj.isDestroyed) {
				...
				(bulletArray as MyArray).removeByIndex(i--);
			} else bulletObj.draw();
		}
	}
}
...
function keyEvent() {
	...
	function setEvent(player: PlayTank, up: number, down: number, left: number, right: number) {
		const { UP, DOWN, LEFT, RIGHT } = DIR;
		switch (!0) {
			case (keys as MyArray).contain(up): setMove(UP); break;
			case (keys as MyArray).contain(down): setMove(DOWN); break;
			case (keys as MyArray).contain(left): setMove(LEFT); break;
			case (keys as MyArray).contain(right): setMove(RIGHT); break;
		}
...
function addEnemyTank() {
	if (enemyArray == null || enemyArray.length >= maxAppearEnemy || maxEnemy == 0)
		return;
	appearEnemy++;
	let obj: Enemy3 | Enemy1 | Enemy2 = null as unknown as (Enemy3 | Enemy1 | Enemy2);
	switch (parseInt(Math.random() * 3 as unknown as string)) {
		case 0: obj = new Enemy1(ctx.tank); break;
		case 1: obj = new Enemy2(ctx.tank); break;
		case 2: obj = new Enemy3(ctx.tank); break;
	}
	obj.x = ENEMY_LOCATION[parseInt(Math.random() * 3 as unknown as string)] + map.offsetX;
	obj.y = map.offsetY;
	obj.dir = DIR.DOWN;
...
function drawEnemyTanks() {
	if (enemyArray != null || (enemyArray as (Enemy1 | Enemy2 | Enemy3)[]).length > 0) {
		for (let i = 0; i < enemyArray.length; i++) {
			...
			enemyObj.isDestroyed ? (enemyArray as MyArray).removeByIndex(i--) : enemyObj.draw();
		}
	}
...
function drawCrack() {
	if (crackArray != null && crackArray.length > 0) {
		for (let i = 0; i < crackArray.length; i++) {
			...
			if (crackObj.isOver) {
				(crackArray as unknown as MyArray).removeByIndex(i--);
...
function gameOver() {
	...
	ctx.over.drawImage(IMAGE.RESOURCE, POS.over[0] as number, POS.over[1] as number, 64, 32, overX + map.offsetX, overY + map.offsetY, 64, 32);
	overY -= 2;
	if (overY <= parseInt(map.mapHeight / 2 as unknown as string)) {
		initObject();
		//只有一个玩家
		menu.playNum == 1 && (player[1].lives = 0);
