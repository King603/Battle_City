 ...
 */
export function CheckIntersect(object1: Obj, object2: Obj, overlap: number): boolean {
	//    x-轴                      x-轴
	//  A1------>B1 C1              A2------>B2 C2
	//  +--------+   ^              +--------+   ^
...
export function tankMapCollision(tank: Tank, mapObj: Map): boolean {
	...
	let { rowIndex, /* map中的行索引 */		colIndex /* map中的列索引 */ } = ((): { rowIndex: number; colIndex: number; } => {
		switch (tank.dir) {
			case UP: return {
				rowIndex: parseInt((y + overlap) / mapObj.tileSize as unknown as string),
				colIndex: parseInt((x + overlap) / mapObj.tileSize as unknown as string),
			};
			case DOWN: return {
				rowIndex: parseInt((y - overlap + tank.size) / mapObj.tileSize as unknown as string),// 向下，即dir==1的时候，行索引的计算需要+tank.Height
				colIndex: parseInt((x + overlap) / mapObj.tileSize as unknown as string),
			};
			case LEFT: return {
				rowIndex: parseInt((y + overlap) / mapObj.tileSize as unknown as string),
			...
			case RIGHT: return {
				rowIndex: parseInt((y + overlap) / mapObj.tileSize as unknown as string),
				colIndex: parseInt((x - overlap + tank.size) / mapObj.tileSize as unknown as string),// 向右，即dir==3的时候，列索引的计算需要+tank.Height
			};
	...
	switch (tank.dir) {
		case UP: case DOWN: {
			let tempWidth = parseInt(tank.tempX - map.offsetX - colIndex * mapObj.tileSize + tank.size - overlap as unknown as string); // 去除重叠部分
			/**需要检测的tile数 */
			let tileNum = parseInt(tempWidth / mapObj.tileSize as unknown as string) + (tempWidth % mapObj.tileSize == 0 ? 0 : 1);
		...
		case LEFT: case RIGHT: {
			let tempHeight = parseInt(tank.tempY - map.offsetY - rowIndex * mapObj.tileSize + tank.size - overlap as unknown as string); // 去除重叠部分
			/**需要检测的tile数 */
			let tileNum = parseInt(tempHeight / mapObj.tileSize as unknown as string) + (tempHeight % mapObj.tileSize == 0 ? 0 : 1);
...
export function bulletMapCollision(bullet: Bullet, mapobj: Map): boolean {
	...
	let { rowIndex,/* map中的行索引 */ colIndex /* map中的列索引 */ } = ((): { rowIndex: number; colIndex: number; } => {
		switch (bullet.dir) {
			case UP: return {
				rowIndex: parseInt(col / mapobj.tileSize as unknown as string),
				colIndex: parseInt(row / mapobj.tileSize as unknown as string)
			};
			case DOWN: return {
				rowIndex: parseInt((col + bullet.size) / mapobj.tileSize as unknown as string), // 向下，即dir==1的时候，行索引的计算需要+bullet.Heigh,
				colIndex: parseInt(row / mapobj.tileSize as unknown as string)
			};
			case LEFT: return {
				rowIndex: parseInt(col / mapobj.tileSize as unknown as string),
			...
			case RIGHT: return {
				rowIndex: parseInt(col / mapobj.tileSize as unknown as string),
				colIndex: parseInt((row + bullet.size) / mapobj.tileSize as unknown as string) // 向右，即dir==3的时候，列索引的计算需要+bullet.Height
			};
			default: return { rowIndex: 0, colIndex: 0 }
		}
	})();
	...
	switch (bullet.dir) {
		case UP: case DOWN: {
			let tempWidth = parseInt(bullet.x - map.offsetX - colIndex * mapobj.tileSize + bullet.size as unknown as string);
			/**需要检测的tile数 */
			let tileNum = parseInt(tempWidth / mapobj.tileSize as unknown as string) + (tempWidth % mapobj.tileSize == 0 ? 0 : 1);
		...
		case LEFT: case RIGHT: {
			let tempHeight = parseInt(bullet.y - map.offsetY - rowIndex * mapobj.tileSize + bullet.size as unknown as string);
			/**需要检测的tile数 */
			let tileNum = parseInt(tempHeight / mapobj.tileSize as unknown as string) + (tempHeight % mapobj.tileSize == 0 ? 0 : 1);
